...
class HyperNetwork(nn.Module):
    def __init__(self, f_size=3, z_dim=64, in_channels=512, out_channels=1, feature_size=16):
        ...
        self.linear_out = nn.Linear(
            in_features=self.z_dim, out_features=self.out_channels * self.f_size * self.f_size).cuda()
    ...
    def forward(self, z):
        ...
        for i in range(self.in_channels):
            ...
            kernel = k.view(batch_size, self.out_channels,
...
if __name__ == '__main__':
    ...
    model = TemplateMatching(
        z_dim=64, output_channel=32, pretrain=False).cuda()
    res = model(x, t)
